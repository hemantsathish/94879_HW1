...
def parse_date_time(record):
    ts = record.get("ts") or record.get("Timestamp") or record.get("dateTime") or record.get("datetime")
    if ts is not None:
    ...
    if date is not None and time is not None:
        ...
        dt = pd.to_datetime(f"{str(date).strip()} {tnorm}", dayfirst=True, errors="coerce")
        if not pd.isna(dt):
            return dt
...
class RollingState:
    ...
    def make_features(self, key, lags=(1, 2, 3, 6, 12, 24), roll_windows=(3, 6, 12, 24)):
        if key not in self.buf:
            return {}
